    ...
    <article className="shell grid gap-12 py-16 sm:py-20 lg:grid-cols-[0.75fr_1.25fr] lg:gap-20">
      <figure className={reverse ? 'lg:order-2' : ''}>
        <div className="relative max-w-sm">
          <div
            className="absolute -bottom-4 -right-4 h-full w-full border border-brass/50"
            aria-hidden
          />
          <img
            src={person.photo}
        ...
        <figcaption className="mt-7 max-w-sm">
          <h2 className="font-display text-2xl leading-tight text-deep">{person.name}</h2>
          <p className="mt-2 text-sm font-medium text-reef">{person.role}</p>
          <p className="mt-4 text-sm leading-relaxed text-ink/60">{person.remit}</p>
          <ul className="mt-6 space-y-2 border-t border-deep/15 pt-5">
        ...
        <div className="space-y-5">
          {person.bio.map((p, i) => (
            <p
              ...
              className={
                i === 0
                  ? 'font-display text-2xl leading-[1.4] text-reef sm:text-[1.7rem]'
                  : 'lede text-ink/75'
              }
            >
              {p}
  ...
  return (
    <>
      ...
      <div className="bg-limestone">
        ...
        <div className="shell">
          <div className="h-px w-full bg-deep/15" aria-hidden />
        </div>
      ...
      <section className="bg-deep py-20 text-limestone sm:py-28">
        <div className="shell">
          ...
          <div className="mt-14 grid gap-x-16 gap-y-14 lg:grid-cols-2">
            {team.map((person) => (
              <article key={person.name} className="border-t border-limestone/20 pt-7">
                <h3 className="font-display text-2xl leading-tight text-limestone">
                  {person.name}
                </h3>
                <p className="mt-2 text-sm font-medium text-brass">{person.role}</p>
                <p className="mt-1.5 text-sm text-seaglass/80">{person.remit}</p>
                {person.credentials.length > 0 && (
                  <ul className="mt-5 space-y-1.5">
                    {person.credentials.map((c) => (
                      <li key={c} className="text-sm leading-relaxed text-seaglass">
                        {c}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="mt-5 space-y-4">
                  {person.bio.map((p, i) => (
                    <p key={i} className="text-[0.95rem] leading-relaxed text-limestone/70">
                      {p}
                    </p>
                  ))}
                </div>
